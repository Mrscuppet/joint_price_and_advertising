...
class Environment:
    def __init__(self,k,t,allocation_mec,user,CTR):
        ...
        self.user = user                          #generatore che genera lo user.
        self.t = t                                                       #time istat considerato che influisce sul calcolo del CTR
        self.CTR = CTR                                                   #vettore che indica CTR di ogni advertiser

    def step(self,comp_prices_bids,agent_price_bid,user_counter):
        #print("Iterazione numero:",self.t - 9)
        u = self.user[user_counter]                               #la lunghezza di user_gen deve essere quanto quella dei competitors +1,restiuisce vettore user con primo elemento indicante quanti batch vede lo user, e gli altri il "quality score" dello user per ogni competitor.
        ...
        if len(index_of_min[0]) > 1:
            ...
            if type(visit[num_ran[0]]) == type(comp_prices_bids[0]):        #Se l'ad scelto è di un competiro allora la reward sarà 0, e ci salviamo l'id dell'agente che ci serve per utilizzo del mab successivamente ( in particolare per EXP3 )
                ...
                self.CTR = ut.CTR_update_regret(self.t,reward_and_id,self.CTR)
                self.t = self.t + 1
